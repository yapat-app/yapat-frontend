...
export type StudyLogEvent =
  ...
  | {
      eventType: "phase_tour_start";
      payload: { phase: string; stepKeys: string[] };
    }
  | {
      eventType: "phase_tour_complete";
      payload: { phase: string; completed: boolean };
    }
  | {
      eventType: "phase_timer_start";
      payload: { phase: string; durationSec: number };
    }
  ...
  // ── Feed ────────────────────────────────────────────────────────────
  | {
      eventType: "feed_active_snippet_change";
      payload: { snippetId: number; source: ScrollSource };
    }
  | {
      eventType: "feed_sort_change";
      payload: {
        action: "sort_change";
        panel: "prediction_feed";
        activeCount: number;
        fields: Array<{ property: string; direction: "asc" | "desc" }>;
      };
    }
  ...
  | {
      ...
      payload: { action: string; labels: string[] };
    }
  | {
      eventType: "label_toggle";
  ...
  | {
      eventType: "vis_point_hover";
      payload: { snippetId: number; projectionMethod: string };
    }
  | {
      eventType: "projection_method_change";
  ...
  // ── Filters ─────────────────────────────────────────────────────────
  | {
      eventType: "visibility_threshold_change";
      payload: { property: string; value: number };
    }
  | {
      eventType: "visibility_range_change";
      payload: { property: string; min: number; max: number };
    }
  | {
  ...
  | {
      eventType: "date_time_filter_change";
      payload: {
        filter: "date" | "time";
        active: boolean;
        min: number | null;
        max: number | null;
        /** Human-readable versions of min/max (e.g. "May 15" or "09:00"). */
  ...
  | {
      ...
      payload: { durationMs: number; modelFamilyName: string | null };
    }
  | {
      eventType: "retrain_failed";
      payload: { modelFamilyName: string | null; error: string };
    }
